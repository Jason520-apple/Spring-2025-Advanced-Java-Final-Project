...
public class OCCCPerson extends RegisteredPerson {
	...
	public OCCCPerson(RegisteredPerson p, String studentID) {
		super(p.getFirstName(), p.getLastName(), p.getDOB(), p.getGovernmentID()); // pass in the parameters of a
																					// registered person to parent
																					// constructor
	...
	// method overloading, for if a registered person, or a person object is passed
	// in for comparison
	public boolean equals(RegisteredPerson p) {
		...
		// must use getters since govID, firstName, and lastName are part of parent
		// classes not directly in this OCCCPerson class
	}

	public boolean equals(Person p) {
		// compare only names and DOB
		return this.getFirstName().equalsIgnoreCase(p.getFirstName())
				&& this.getLastName().equalsIgnoreCase(p.getLastName()) && this.getDOB().equals(p.getDOB());
	}
